...
#define BATTERY_DEBOUNCE_MS 2000
/* USB draw curren*/
...
struct pm8921_chg_chip {
	...
	struct wake_lock		usbin_wake_lock;  // USB in/out wakelock
	struct mutex			mutex_lock;
	struct delayed_work		batt_missing_work;  // Battry missing check work
};
...
static void batt_missing_worker(struct work_struct *work)
{
	...
	if (!get_prop_batt_present(chip)) {
		...
		if (batt_temp >= 480 && acer_sku_wi_gauge) {  //Gauge Hot
			is_battery_hot = true;
		} else if (batt_temp >= 430 && !acer_sku_wi_gauge) {  //BMS Hot
			is_battery_hot = true;
		} else if (batt_temp <= 20) {  //Cold
			if (batt_temp <= -400) {  //Battery missing(Temp=-412)
				pr_info("Battery missing!!(Temp)\n");
				is_battery_missing = true;
				power_supply_changed(&chip->usb_psy);
				power_supply_changed(&chip->dc_psy);
			} else {
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (usb_target_ma > USB_WALL_THRESHOLD_MA) {
			...
			if (val->intval) {
				if (get_prop_batt_status(the_chip) ==
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (usb_target_ma <= USB_WALL_THRESHOLD_MA) {
			val->intval = is_usb_chg_plugged_in(the_chip);
			/* Fix battery icon abnormal when PC enter standby with USB connected */
			if (val->intval) {
				if (get_prop_batt_status(the_chip) ==
								POWER_SUPPLY_STATUS_DISCHARGING)
					val->intval = 0;
...
static int gauge_soc_check(struct pm8921_chg_chip *chip, int soc)
{
	...
	if (is_usb_chg_plugged_in(chip)) {  //Charging
		...
		//Prevent battery level rising, Bug 13691.
		if (percent_soc > chip->recent_reported_soc)
			percent_soc = chip->recent_reported_soc;
	}
...
static int get_prop_batt_temp(struct pm8921_chg_chip *chip)
{
	...
	if (pm8921_batt_gauge) {
		int batt_temp = pm8921_batt_gauge->get_battery_temperature();
		/* battery missing */
		if (batt_temp <= -400)
			is_battery_missing = true;
		return batt_temp;
...
#if defined(CONFIG_ACER_INPUT)
void acer_notify_usb_of_the_plugin_event(int plugin)
{
	notify_usb_of_the_plugin_event(plugin);
}
EXPORT_SYMBOL_GPL(acer_notify_usb_of_the_plugin_event);
#endif
...
static void handle_usb_insertion_removal(struct pm8921_chg_chip *chip)
{
	...
	if (usb_present && is_mhl_charge == 0) {
		/* enable smart charger after 2 second */
...
static void vin_collapse_check_worker(struct work_struct *work)
{
	...
	if (is_usb_chg_plugged_in(chip) &&
		...
		pm8921_charger_vbus_draw(USB_WALL_THRESHOLD_MA);
...
static bool is_first_ma_decrease = false; //Workaround for Bug17850
static void unplug_check_worker(struct work_struct *work)
{
	...
	if (active_path & USB_ACTIVE_BIT) {
		...
		if ((reg_loop & VIN_ACTIVE_BIT) &&
			...
			&& !charging_disabled) {
			/* Workaround for Bug17850.
			 * DUT can not resume immediately after unplug AC
			 * under suspend on PVT device.
			 */
			if (is_first_ma_decrease) {
				pm8921_charger_vbus_draw(0);
				is_first_ma_decrease = false;
				pr_info("set vbus draw to 0mA\n");
			}
	...
	if (!(reg_loop & VIN_ACTIVE_BIT) && (active_path & USB_ACTIVE_BIT)
		...
		if (usb_ma < usb_target_ma) {
			...
			is_first_ma_decrease = true; //Workaround for Bug17850
			usb_target_ma = usb_ma;
		}
	}
check_again_later:
...
static void update_heartbeat(struct work_struct *work)
{
	...
	if (pm8921_batt_gauge) {
		int temp = 0;
		// Refresh Gauge data
		pm8921_batt_gauge->bq27520_status_refresh();
		temp = pm8921_batt_gauge->get_battery_temperature();
		...
		pr_info("Cap=%d%%(%d) Volt=%dmV Amp=%dmA Temp=%d.%ddegC SoC=%d%%\n",
				chip->recent_reported_soc, soc_offset_cur,
...
static bool is_vbatlow_changed = false;
static int is_charging_finished(struct pm8921_chg_chip *chip)
{
	...
	if (!is_ext_charging(chip)) {
		...
		pr_debug("fast_chg = %d\n", fast_chg);
		if (fast_chg == 0) {
			/* Workaround for Bug17870.
			 * DUT will stop charging sometimes if battery voltage
			 * is over resume voltage.
			 * When this issue happens, set resume voltage to 4200mV
			 * to re-enable charging process.
			 */
			if (get_prop_batt_capacity(chip) < 100 &&
				pm_chg_get_rt_status(chip, BAT_TEMP_OK_IRQ) &&
				!pm_chg_get_rt_status(chip, VBATDET_LOW_IRQ) &&
				is_usb_chg_plugged_in(chip)) {
				if (!is_vbatlow_changed) {
					pm_chg_vbatdet_set(chip, chip->max_voltage_mv);
					pr_info("not full, restart charging\n");
					is_vbatlow_changed = true;
				}
				return CHG_IN_PROGRESS;
			} else {
				/* Workaround for Bug17870, reset resume voltage */
				if (is_vbatlow_changed) {
					pr_info("reset vbatdet low voltage\n");
					pm_chg_vbatdet_set(chip,
					chip->max_voltage_mv - chip->resume_voltage_delta);
					is_vbatlow_changed = false;
				}
 ...
 * happened
 */
#define CONSECUTIVE_COUNT	12  // Taper window = 120s
static void eoc_worker(struct work_struct *work)
{
	...
	pm_chg_failed_clear(chip, 1);
	if (pm8921_batt_gauge) {
		/* Force gauge driver updating battery status */
		pm8921_batt_gauge->bq27520_status_refresh();
	}
	...
	/* Check battery capacity before end of charge */
	if (pm8921_batt_gauge) {
				...
				struct kobj_attribute *attr, char * buf)
{
	if (is_vbus_disable) {
		...
		/* re-schedule unplug_check_work to start smart charger */
		if (!delayed_work_pending(&the_chip->unplug_check_work))
...
static int pm8921_charger_resume(struct device *dev)
{
	...
	/* update battery status after resume */
	power_supply_changed(&chip->batt_psy);
	/* check battery update work */
...
static int __devinit pm8921_charger_probe(struct platform_device *pdev)
{
	...
	chip->recent_reported_soc = 100;  //Set default SoC to 100
	chip->dev = &pdev->dev;
