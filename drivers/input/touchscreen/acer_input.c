...
void acer_notify_usb_of_the_plugin_event(int);
#endif
...
{
#if defined(CONFIG_MACH_ACER_A9)
	acer_notify_usb_of_the_plugin_event(en);
#endif
}
...
{
	int x = 0, y = 0, major = 0, track_id = 0, press = 0;
	...
	while (1) {
		...
		if (type == EV_KEY) {
			...
			if (code == ABS_MT_POSITION_X ) {
				...
				input_report_abs(input_dev, ABS_MISC, value);
			} else if (code == ABS_MT_WIDTH_MAJOR) {
				input_report_abs(input_dev,ABS_MT_WIDTH_MAJOR, value);
			} else {
				printk("Unknown code =%d\n", code);
			}
			continue;
			...
			if (code == 2) {
#if INPUT_NEED_DEBUG
				printk("x=%d, y=%d, major=%d, track_id=%d\n", x, y, major, track_id);
...
static int __devinit acer_input_probe(struct platform_device *pdev)
{
	...
	set_bit(ABS_MT_POSITION_X, input_dev->keybit);
	set_bit(ABS_MT_POSITION_Y, input_dev->keybit);
	set_bit(ABS_MT_TOUCH_MAJOR, input_dev->keybit);
	set_bit(ABS_MT_PRESSURE, input_dev->keybit);
	set_bit(KEY_PLAY_FINISH, input_dev->keybit);
	//set_bit(ABS_MT_WIDTH_MAJOR, input_dev->keybit);
	...
	input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, ABS_PRESSURE_MAX, 0, 0);
