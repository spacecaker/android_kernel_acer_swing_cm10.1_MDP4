...
unsigned int app_killed_count[MAX_HASH_SIZE];
#endif
...
#if defined(CONFIG_MACH_ACER_A9)
int hash_app_killed(const char *name)
{
	int i;
	unsigned int sum=0;
	...
	for_each_process(p) {
		...
		if (selected) {
			...
			if (oom_adj == selected_oom_adj &&
				...
				continue;
#if defined(CONFIG_MACH_ACER_A9)
			if (app_killed_count[hash_app_killed(p->comm)] > app_killed_count[hash_app_killed(selected->comm)])
				continue;
#endif
		}
		selected = p;
...
#if defined(CONFIG_MACH_ACER_A9)
		app_killed_count[hash_app_killed(selected->comm)]++;
#endif
	}
