...
#ifdef CONFIG_MACH_ACER_A11RD /* for A11 */
...
static bool mic1_vdd_l6_on = false;
static struct regulator *mic1_vdd_l6;
#endif
...
#ifdef CONFIG_MACH_ACER_A11RD
static int mic1bias_vreg_enable(void)
{
	int rc = 0;

	rc = regulator_set_optimum_mode(mic1_vdd_l6, 600000);
	if (rc < 0) {
		pr_err("%s: regulator_set_optimum_mode failed rc=%d\n",
			__func__, rc);
	}

	rc = regulator_set_voltage(mic1_vdd_l6, 2950000, 2950000);
	if (rc) {
	...
	rc = regulator_enable(mic1_vdd_l6);
	if (rc) {
...
static int mic1bias_vreg_disable(void)
{
	int rc = 0;
	...
	rc = regulator_disable(mic1_vdd_l6);
	if (rc) {
		pr_err("%s: regulator_enable failed rc =%d\n",
			__func__, rc);
		goto error_vdd;
	...
	rc = regulator_set_optimum_mode(mic1_vdd_l6, 0);
	if (rc < 0) {
		pr_err("%s: regulator_set_optimum_mode failed rc=%d\n",
			__func__, rc);
	}

	rc = regulator_set_voltage(mic1_vdd_l6, 0, 2950000);
	if (rc) {
		pr_err("%s: regulator_set_voltage"
			"failed rc =%d\n", __func__, rc);
		goto error_vdd;
	...
	mic1_vdd_l6_on = false;

error_vdd:
	return rc;
}

static int msm8960_intmic_event(struct snd_soc_dapm_widget *w,
        struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		mic1bias_vreg_enable();
		pm8xxx_hsed_bias_control(PM8XXX_HSED_BIAS1, 1);
		snd_soc_update_bits(codec, TABLA_A_MICB_1_INT_RBIAS, 1 << 4, 0);
		snd_soc_update_bits(codec, TABLA_A_MICB_3_INT_RBIAS, 1 << 7, 0);
	} else {
		mic1bias_vreg_disable();
...
static int msm8960_extmic_event(struct snd_soc_dapm_widget *w,
        struct snd_kcontrol *k, int event)
{
	pr_debug("%s() %x\n", __func__, SND_SOC_DAPM_EVENT_ON(event));
...
static const struct snd_soc_dapm_widget msm8960_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SPK("Ext Spk Top", NULL),
#else
...
#ifdef CONFIG_MACH_ACER_A11RD
	SND_SOC_DAPM_MIC("Handset Mic", msm8960_intmic_event),
	SND_SOC_DAPM_MIC("Headset Mic", msm8960_extmic_event),
#else
...
#endif
#ifdef CONFIG_MACH_ACER_A11RD
	/* for A11 #ifdef CONFIG_MACH_ACER_A9 */
	mic1_vdd_l6 = regulator_get(NULL, "8921_l6");
	if (IS_ERR(mic1_vdd_l6)) {
		ret = PTR_ERR(mic1_vdd_l6);
